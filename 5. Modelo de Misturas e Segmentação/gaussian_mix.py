...
class BaysianClassifier:
    ...
    def fit(self, train, k):
        ...
        tuples = []
        for c in classes:
            self.mixtureGaussians = GMM(k, 25)
            train_ = train[np.where(train[:,-1] == c)] #Separa os dados por classe
            prob_ = float(len(train_))/len(train) #Calcula as probabilidades a priori
            tuples.append((prob_, train_))
            
            self.prob_gaussians.append(self.mixtureGaussians.fit(train_[:,:-1]))
            #print self.prob_gaussians
    ...
    def likelihood(self, x, c, l):
        ...
        (mu, sigma, pi) = self.prob_gaussians[c]
        
        groups_probabilities = self.mixtureGaussians.predict_proba_x(x, mu, sigma, pi)
        #print " "
        #print groups_probabilities
        #print " " 
        return np.sum(groups_probabilities) 
    ...
    def predict(self, x):
        ...
        for c in self.classes:
            c = int(c)
            priori = self.tuples[c][0] #probabilidade a priori da classe c
            posteriori = self.likelihood(x, c, l) * priori #Probabilidade a posteriori da classe c
...
if __name__ == "__main__":
    ...
    for realization in range(realizations):
        ...
        #baysianClassifier.prob_gaussians = []
        k_ = [2,3,4]
        classifiers = []
        for k in k_:
            baysianClassifier = BaysianClassifier()
            start_train = time.time()
            parameters = baysianClassifier.fit(train, k)
            classifiers.append(baysianClassifier)
            end_train = time.time()

            times_train.append(end_train - start_train)
            #k_ = [x for x in range(2,5)]
            accs_train = []
        ...
        k_best = k_[index_best]
        baysianClassifier = classifiers[index_best]
        ...
        models.append((train, test, k_best))
